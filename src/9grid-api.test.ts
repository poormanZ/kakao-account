...
describe("9Grid action adapter", () => {
  it("validates and parses supported actions", () => {
    ...
    expect(parseNineGridAction({ type: "reroll", indexes: [0, 2] })).toEqual({ type: "reroll", indexes: [0, 2] });
  ...
  it("does not execute an action when its state-machine precondition fails", () => {
    ...
    expect(() => applyNineGridAction(state, { type: "select", cardId: "a" }, { generateCards })).toThrow("Card cannot be selected");
  });

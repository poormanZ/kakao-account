...
export const renderPortalPage = (user: WebUser | null, games: GameCatalogItem[], error?: string | null): string => {
  const cards = games.slice().sort((a,b)=>a.sort_order-b.sort_order).map((game)=>{
    ...
    return `<article class="game-card"><div><div class="game-icon" aria-hidden="true">${escapeHtml(game.icon)}</div><h3>${escapeHtml(game.name)}</h3><p>${escapeHtml(game.description)}</p>${game.status === "coming_soon" ? `<span class="status">COMING SOON</span>` : ""}</div><div class="card-actions">${play}${ranking}</div></article>`;
  }).join("");
...
const render9GridRankingPage = (user: WebUser | null): string => shell("9Grid 랭킹",`<main><section class="hero"><p class="eyebrow">RANKING · 9GRID</p><h1>9Grid 랭킹</h1><p class="subtitle">최대 클리어 라운드 → 클리어 턴 → 남은 HP 순으로 순위를 계산합니다.</p></section><section class="game-card" style="min-height:0"><div id="ranking" class="notice" style="margin-top:0">랭킹을 불러오는 중...</div><div id="myRank" class="notice" style="margin-top:12px">내 순위를 확인하는 중...</div></section><p style="margin-top:20px;display:flex;gap:8px;flex-wrap:wrap"><a class="action secondary" href="/games/9grid">게임으로 돌아가기</a><a class="action secondary" href="/">← 게임 목록으로</a></p><script>
const ranking=document.getElementById("ranking");const myRank=document.getElementById("myRank");
const escapeText=(value)=>String(value).replace(/[&<>']/g,(character)=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[character]||character));
const loadRanking=async()=>{try{const response=await fetch("/api/games/9grid/ranking?limit=100");const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||"request failed");const rows=Array.isArray(data.scores)?data.scores:[];const header='<div style="font-weight:800;margin-bottom:12px">TOP '+rows.length+'</div>';const items=rows.map((row,index)=>'<div style="display:grid;grid-template-columns:52px 1fr auto;gap:12px;padding:10px 0;border-bottom:1px solid #eee;align-items:center"><strong>'+(index+1)+'</strong><span>'+escapeText(row.nickname||("USER #"+row.account_user_id))+'</span><span>R'+Number(row.max_round)+' · T'+Number(row.last_round_clear_turn)+' · HP '+Number(row.remaining_hp)+'</span></div>').join("");ranking.innerHTML=rows.length?header+items:'<div>아직 등록된 기록이 없습니다.</div>';}catch{ranking.textContent="랭킹 정보를 불러오지 못했습니다.";}};
const loadMine=async()=>{try{const response=await fetch("/api/games/9grid/my-rank");const data=await response.json().catch(()=>({}));if(response.status===401){myRank.textContent="로그인하면 내 순위를 확인할 수 있습니다.";return;}if(!response.ok)throw new Error(data.error||"request failed");myRank.textContent=data.rank?"내 순위: "+Number(data.rank)+"위":"아직 내 기록이 없습니다.";}catch{myRank.textContent="내 순위를 불러오지 못했습니다.";}};
void loadRanking();void loadMine();
</script></main>`,user);

export const renderGamePlaceholderPage = (user: WebUser | null, game: GameCatalogItem, ranking = false): string => {
  if (ranking && game.slug === "9grid") return render9GridRankingPage(user);
  return shell(ranking ? `${game.name} 랭킹` : game.name,`<main><section class="hero"><p class="eyebrow">${ranking ? "RANKING" : "GAME"}</p><h1>${escapeHtml(ranking ? `${game.name} 랭킹` : game.name)}</h1><p class="subtitle">${escapeHtml(game.description)}</p></section><div class="notice">${ranking ? "랭킹 데이터 모델과 점수 검증을 구현한 뒤 실제 순위가 표시됩니다." : "게임 플레이 화면은 다음 구현 단계에서 연결됩니다."}</div><p style="margin-top:20px"><a class="action secondary" href="/">← 게임 목록으로</a></p></main>`,user);
};
...
const f=document.getElementById("nicknameForm"),i=document.getElementById("nickname"),b=document.getElementById("saveButton"),c=document.getElementById("currentNickname"),m=document.getElementById("message"),l=document.getElementById("logoutAccountButton");const msg=(t,k)=>{m.textContent=t;m.className="message "+k};f.addEventListener("submit",async e=>{e.preventDefault();const nickname=i.value.trim();if(nickname.length<2||nickname.length>20){msg("닉네임은 2~20자로 입력해주세요.","error");return}b.disabled=true;try{const r=await fetch("/api/account",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({nickname})});const d=await r.json().catch(()=>({}));if(!r.ok){msg(d.error||"저장에 실패했습니다.","error");return}c.textContent=d.nickname||nickname;msg("저장되었습니다.","success");}catch{msg("저장에 실패했습니다.","error");}finally{b.disabled=false}});l.addEventListener("click",async()=>{l.disabled=true;try{const r=await fetch("/auth/logout",{method:"POST"});if(r.ok)location.href="/";else l.disabled=false}catch{l.disabled=false}});

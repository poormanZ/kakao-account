...
const loadRanking=async()=>{try{const response=await fetch("/api/games/9grid/ranking?limit=100");const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||"request failed");const rows=Array.isArray(data.scores)?data.scores:[];ranking.innerHTML=rows.length?`<div style="font-weight:800;margin-bottom:12px">TOP ${rows.length}</div>${rows.map((row,index)=>`<div style="display:grid;grid-template-columns:52px 1fr auto;gap:12px;padding:10px 0;border-bottom:1px solid #eee;align-items:center"><strong>${index+1}</strong><span>${escapeText(row.nickname||("USER #"+row.account_user_id))}</span><span>R${Number(row.max_round)} · T${Number(row.last_round_clear_turn)} · HP ${Number(row.remaining_hp)}</span></div>`).join("")}`:`<div>아직 등록된 기록이 없습니다.</div>`;}catch{ranking.textContent="랭킹 정보를 불러오지 못했습니다.";}};

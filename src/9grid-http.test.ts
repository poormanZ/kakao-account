...
const post = (body: unknown) => new Request("https://example.com/api/games/9grid/session/action", {
  method: "POST",
...
describe("9Grid HTTP session", () => {
  ...
  it("returns 409 when an action loses an optimistic-lock race", async () => {
    const db = new FakeDb();
    const env = createEnv(db);
    const started = await handleNineGridSession(post({ type: "start" }), env, 7, "action");

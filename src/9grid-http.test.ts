  ...
  async first<T>(): Promise<T | null> {
    if (this.query.startsWith("SELECT")) {
      ...
      return stateJson === null
        ? null
        : ({ state_json: stateJson, version: this.db.getVersion() } as T);
    }
    return null;
  }
...
describe("9Grid HTTP session", () => {
  ...
  it("returns 409 when an action loses an optimistic-lock race", async () => {
    const db = new FakeDb();
    const env = createEnv(db);
    const started = await handleNineGridSession(
      post({ type: "start" }),
      env,
      7,
      "action",
    ...
    const rerolled = await handleNineGridSession(
      post({ type: "reroll" }),
      env,
      7,
      "action",
    ...
    const storedState = JSON.parse(db.getState() ?? "null") as ReturnType<

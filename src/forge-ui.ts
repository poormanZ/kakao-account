...
function shopKey(){return session.shopWeapons.map((weapon)=>weapon.id+':'+weapon.baseDamage).join('|');}
function skillsKey(){return JSON.stringify(session.skills);}
function renderShop(){const shop=$('shop');if(!shop)return;shop.innerHTML='';session.shopWeapons.forEach((weapon)=>{const card=document.createElement('div');card.className='weapon-card';card.innerHTML='<strong>'+escText(weapon.name)+'</strong><div class="stat">BASE DMG '+weapon.baseDamage+'</div><div class="stat">PRICE '+weapon.baseDamage+' G</div>';const button=document.createElement('button');button.type='button';button.textContent='BUY';button.addEventListener('click',()=>act('BUY_WEAPON',{weaponId:weapon.id}));card.appendChild(button);shop.appendChild(card);});renderedShopKey=shopKey();}
function updateShopButtons(){document.querySelectorAll('#shop button').forEach((button,index)=>{const weapon=session.shopWeapons[index];button.disabled=busy||!weapon||session.gold<weapon.baseDamage;});}
function renderSkills(){const skills=$('skills');if(!skills)return;skills.innerHTML='';['enhancementBonusLevel','greatSuccessLevel','sellBonusLevel'].forEach((skill)=>{const level=session.skills[skill];const base=skill==='enhancementBonusLevel'?25:skill==='greatSuccessLevel'?50:30;const cost=base*2**level;const card=document.createElement('div');card.className='skill';card.innerHTML='<div class="skill-row"><span class="skill-name">'+formatSkill(skill)+'</span><span>LV '+level+'</span></div><div class="skill-desc">'+(skill==='enhancementBonusLevel'?'강화 성공 확률 +1%p':skill==='greatSuccessLevel'?'+2 강화 확률 +1%p':'판매가 +1%')+'</div>';const button=document.createElement('button');button.type='button';button.textContent=level>=20?'MAX':'BUY · '+cost+' G';button.addEventListener('click',()=>act('BUY_SKILL',{skill}));card.appendChild(button);skills.appendChild(card);});renderedSkillsKey=skillsKey();}
function updateSkillButtons(){document.querySelectorAll('#skills button').forEach((button,index)=>{const skill=['enhancementBonusLevel','greatSuccessLevel','sellBonusLevel'][index];const level=session.skills[skill];button.disabled=busy||level>=20||session.gold<(skill==='enhancementBonusLevel'?25:skill==='greatSuccessLevel'?50:30)*2**level;});}
function updateActionButtons(){const upgrade=$('upgrade');const sell=$('sell');const refresh=$('refresh');if(upgrade){upgrade.disabled=busy||!session.currentWeapon||session.currentWeapon.enhancementLevel>=10||session.gold<session.currentWeapon.enhancementLevel;upgrade.textContent=busy?'HAMMER // WORKING...':'HAMMER // UPGRADE';}if(sell){sell.disabled=busy||!session.currentWeapon;sell.textContent='SELL WEAPON';}if(refresh){refresh.disabled=busy;refresh.textContent=busy?'REFRESHING...':'REFRESH SHOP';}}
...
if(shopKey()!==renderedShopKey)renderShop();if(skillsKey()!==renderedSkillsKey)renderSkills();updateShopButtons();updateSkillButtons();updateActionButtons();}

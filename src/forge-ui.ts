...
async function json(url,options){const response=await fetch(url,options);const body=await response.json().catch(()=>null);if(!response.ok){const error=new Error(body?.error||'REQUEST FAILED');error.status=response.status;error.version=body?.version;throw error;}return body;}

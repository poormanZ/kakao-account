...
describe("9Grid combat", () => {
  it("uses Warrior synergy level for multiplied attack", () => {
    const result = calculateCombatStats({ lines: [], races: {}, jobs: { warrior: 1 } });
    expect(result.attack).toBe(4);
  });

  it("uses Tank synergy level for multiplied defense", () => {
    const result = calculateCombatStats({ lines: [], races: {}, jobs: { tank: 2 } });
  ...
  it("uses Mage synergy level for mana-based combat power", () => {
    const result = calculateCombatStats({ lines: [], races: {}, jobs: { mage: 3 } });
  ...
  it("combines job and race effects during combat", () => {
    const synergy = findBoardSynergy(line("elf", "warrior"));
    const result = calculateCombat({ synergy, playerHp: 100, playerMaxHp: 100, monsterHp: 100, monsterAttack: 1 });
  ...
  it("marks game defeat when post-combat HP reaches zero", () => {
    const result = calculateCombat({ synergy: { lines: [], races: {}, jobs: {} }, playerHp: 5, playerMaxHp: 100, monsterHp: 100, monsterAttack: 20 });

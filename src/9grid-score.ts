...
const parseScoreBody = async (request: Request): Promise<{
  max_round: number;
  last_round_clear_turn: number;
  remaining_hp: number;
} | null> => {
  ...
  try {
    ...
    return {
      max_round: maxRound,
      last_round_clear_turn: clearTurn,
...
export const save9GridScore = async (
  request: Request,
  env: ScoreEnv,
  user: AuthUser,
): Promise<Response> => {
  ...
  try {
    await dbForScores(env)
      ...
      .bind(user.id, body.max_round, body.last_round_clear_turn, body.remaining_hp)
      .run();
...
export const get9GridBestScore = async (env: ScoreEnv, user: AuthUser): Promise<Response> => {
  try {
    const row = await dbForScores(env)
      .prepare(`SELECT id, account_user_id, max_round, last_round_clear_turn, remaining_hp, created_at FROM "9grid_scores" WHERE account_user_id = ? ORDER BY max_round DESC, last_round_clear_turn ASC, remaining_hp DESC, created_at ASC LIMIT 1`)
      .bind(user.id)
      .first<ScoreRow>();
...
export const get9GridMyRank = async (env: ScoreEnv, user: AuthUser): Promise<Response> => {
  try {
    const best = await dbForScores(env)
      .prepare(`SELECT max_round, last_round_clear_turn, remaining_hp, created_at FROM "9grid_scores" WHERE account_user_id = ? ORDER BY max_round DESC, last_round_clear_turn ASC, remaining_hp DESC, created_at ASC LIMIT 1`)
      .bind(user.id)
      .first<Pick<NineGridScore, "max_round" | "last_round_clear_turn" | "remaining_hp" | "created_at">>();

    if (!best) return scoreJson({ rank: null, score: null });

    const rankRow = await dbForScores(env)
      .prepare(`SELECT COUNT(*) + 1 AS rank FROM "9grid_scores" WHERE max_round > ? OR (max_round = ? AND last_round_clear_turn < ?) OR (max_round = ? AND last_round_clear_turn = ? AND remaining_hp > ?) OR (max_round = ? AND last_round_clear_turn = ? AND remaining_hp = ? AND created_at < ?)`)
      .bind(
        best.max_round,
        best.max_round,
        best.last_round_clear_turn,
        best.max_round,
        best.last_round_clear_turn,
        best.remaining_hp,
        best.max_round,
...
export const get9GridRanking = async (env: ScoreEnv, limit = 20): Promise<Response> => {
  ...
  try {
    const result = await dbForScores(env)
      .prepare(`SELECT s.id, s.account_user_id, u.nickname, s.max_round, s.last_round_clear_turn, s.remaining_hp, s.created_at FROM "9grid_scores" s INNER JOIN users u ON u.id = s.account_user_id ORDER BY s.max_round DESC, s.last_round_clear_turn ASC, s.remaining_hp DESC, s.created_at ASC LIMIT ?`)
      .bind(safeLimit)
      .all();

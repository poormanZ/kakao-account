...
const kakaoError = (headers: Headers, status = 502) =>
  json({ error: "Kakao authentication failed" }, { status, headers });
...
const getSessionUser = async (db: D1Database, sessionId: string): Promise<SessionUserRow | null> => {
  ...
  const session = await db
    .prepare(
      `SELECT u.id, u.kakao_user_id, u.nickname, u.profile_image_url, s.expires_at
       FROM sessions s
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && url.pathname === "/auth/kakao/callback") {
      ...
      try {
        ...
        if (typeof user.id !== "number") return kakaoError(headers);

        const internalUser = await getOrCreateUser(env.DB, String(user.id));
        const sessionId = await createSession(env.DB, internalUser.id);
        headers.set("Set-Cookie", [
          clearCookie(STATE_COOKIE, secure),
          sessionCookie(sessionId, secure),
        ].join(", "));

        return json(
          { authenticated: true, provider: "kakao", user: { id: internalUser.id } },
          { headers },
        );
      } catch {
    ...
    if (request.method === "GET" && url.pathname === "/api/me") {
      ...
      try {
        ...
        if (!user) {
          return json(
            { error: "Unauthorized" },
            { status: 401, headers: { "Set-Cookie": clearCookie(SESSION_COOKIE, secure) } },
          );
        }

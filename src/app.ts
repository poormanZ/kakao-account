...
const app = {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && url.pathname === "/api/games/click-rush/ranking") { try { return getClickRushRanking(env.GAME_DB, durationFromRequest(url)); } catch (error) { logError("game.ranking_failed", error, { route: url.pathname, method: request.method }); return apiJson({ error: "Ranking service unavailable" }, 503); } }

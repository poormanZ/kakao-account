...
import { applyNineGridAction, createDefaultCardGenerator, parseNineGridAction, type NineGridAction } from "./9grid-api";
import { getMonsterAttack as getRoundMonsterAttack } from "./9grid-balance";
import { create9GridSession, load9GridSession, load9GridSessionRecord, NineGridSessionConflictError, update9GridSession, type NineGridSessionEnv } from "./9grid-session";
...
const applyAction = (state: GameState, action: NineGridAction, env: NineGridHttpEnv): GameState => applyNineGridAction(state, action, { generateCards: createDefaultCardGenerator(), monsterAttack: getMonsterAttack(env, state.round.round) });
...
export const handleNineGridSession = async (request: Request, env: NineGridHttpEnv, userId: number, route: NineGridSessionRoute): Promise<Response> => {
  ...
  try { action = parseNineGridAction(body); } catch { return json({ error: "Invalid 9Grid action" }, 400); }
  try {
    ...
    const saved = stored === null ? await create9GridSession(env, userId, nextState) : await update9GridSession(env, userId, nextState, stored.version);
    return json({ state: saved.state, monsterAttack: getMonsterAttack(env, saved.state.round.round) });
  } catch (error) {

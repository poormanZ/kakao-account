...
## Phase 8 — 9Grid 미니게임
- [x] 9Grid 핵심 게임 규칙/루프 설계
- [x] 3×3 카드 보드 설계
- [x] 기존 4속성/4직업 카드 시스템 설계
- [x] 리롤/카드 선택 규칙 설계
- [x] 가로/세로 시너지 규칙 설계
...
- [x] 9Grid 종족(Race) + 직업(Job) 카드 구조 및 MVP 규칙 문서화
- [x] 4종 종족 확정 — 고블린/엘프/드워프/드래곤
- [x] 4종 직업 확정 — 전사/탱커/힐러/법사
- [x] 기본 스탯 확정 — 공격력/방어력/생명력/마나
- [x] 직업 카드 배치 시 영구 스탯 +1 규칙 확정
- [x] 한 줄 완성 기반 종족/직업 시너지 규칙 확정
- [x] 시너지 LV0~LV5 및 `LV + 1` 배율 규칙 확정
- [x] 종족/직업 시너지 효과 확정
- [x] Race/Job 타입 및 카드 모델 전환 구현
- [x] Race/Job 시너지 계산기 전환 및 테스트
- [x] 종족/직업 정의 데이터 구조 및 효과 시스템 구현
- [x] 9Grid UI 카드/시너지 표시 개편 — 참고 화면 기반 상태 계층/가독성/반응형 레이아웃 적용
- [x] 실제 플레이 밸런싱 1차 검증 — all-Warrior 기준 Round 1 6턴 클리어, 의도적 no-synergy 혼합 빌드는 9턴 내 미클리어 테스트 추가
- [ ] 실제 브라우저 플레이 기반 밸런싱 및 UX 검증 — 라운드별 평균 클리어 턴/패배율/모바일 가독성 수집 후 수치 재조정

## Phase 9 — Escape Room : PRIME WORLD
- [x] 전체 스토리/퍼즐 흐름 설계
- [x] 문제 1 병렬 카드 구조 설계
- [x] 문제 1 카드 기반 UI 구현 — 색상 문장 + Performance 그래프
- [x] Performance 그래프 x=1~12 및 소수 위치 색상 점 구현
- [x] 문제 1 ID/비밀번호 병렬 입력 및 서버 검증 구현
- [x] 문제 1 관련 단위 테스트 추가
- [ ] 문제 1 브라우저 실플레이 검증 및 Production secret `PRIME|235711` 설정 확인
- [ ] 생일 기록/외부인 퍼즐 구현
- [ ] 이민수 다이어리 구현
- [ ] 사색석 4인 병렬 퍼즐 구현
- [ ] 저울/중앙 금고 퍼즐 구현
